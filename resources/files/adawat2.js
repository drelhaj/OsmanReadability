$().ready(function() {
  ...
  $('#btn1').click( function() {
 	...
 	$('#random').click(function() {
      $.getJSON(script+",  function(d){
     // $("#InputText").text(d.result+"Taha");
	  document.NewForm.InputText.value=d.result;
        //"#result").text(d.time);
      });	
	});	 
	
	/*$('#random').click(function() {
      $.getJSON("http://maqola.org/site/widget?nolayout", {text:"",}, function(data){
     // $("#InputText").text(d.result+"Taha");
	  document.NewForm.InputText.value="TZA";
        //"#result").text(d.time);
      });	
	});	*/ 
...
"<th>اللاحقة</th>", "<th>الحالة الإعرابية</th>","<th>الجذر</th>", "<th>النوع</th><th>النحوي</th>",
				"<th>شيوع</th>",
				"</tr>"
				].join('');
...
//		$("#result").append(+"  "++"<br/>" );
  ...
  $('#btn2').click( function() {
   ...
   $('#tashkeel').click( function() {
      ...
      $.getJSON(script+"/ajaxGet", {text:textlist[i],action:"Tashkeel2", order:i.toString()}, function(d){
      //$("#result").html("<p class=\'tashkeel\'>"+d.result+"</p>");
	   console.log(d);
